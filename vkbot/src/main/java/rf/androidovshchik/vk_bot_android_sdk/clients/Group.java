package rf.androidovshchik.vk_bot_android_sdk.clients;
...
import rf.androidovshchik.vk_bot_android_sdk.utils.vkapi.CallbackApiHandler;
import rf.androidovshchik.vk_bot_android_sdk.utils.vkapi.CallbackApiSettings;
...
public class Group extends Client {
    ...
    public void uploadCover(String cover, Callback<Object> callback) {
        ...
        if (this.getId() == null || this.getId() == 0) {
            Timber.e("Please, provide group_id when initialising the client, because it's impossible to upload cover to group not knowing it id.");
            return;
        }
        ...
        if (coverFile.exists()) {
            try {
                ...
                Timber.e("Cover file was exists, but IOException occured: {} %s", ignored.toString());
                return;
            }
        } else {
            URL coverUrl;
            try {
                ...
                Timber.e("Bad string was proviced to uploadCocver method: path to file or url was expected, but got this: {}, error: {} %s %s", cover, ignored.toString());
                return;
            }
        }
     ...
     */
    private void updateCoverByFile(byte[] bytes, Callback<Object>... callback) throws JSONException {
                ...
                if (coverUploadedResponse.has("hash") && coverUploadedResponse.has("photo")) {
                    ...
                    if (sync) {
                        ...
                        if (responseS.toString().length() < 10 || responseS.toString().contains("error")) {
                            Timber.e("Some error occured, cover not uploaded: {} %s", responseS.toString());
                        }
                        ...
                        api().call("photos.saveOwnerCoverPhoto", params_saveCover, response1 -> {
                            ...
                            if (response1.toString().length() < 10 || response1.toString().contains("error")) {
                                Timber.e("Some error occured, cover not uploaded: {} %s", response1.toString());
                            }
                            if (callback.length > 0) {
                    ...
                    Timber.e("Error occured when uploading cover: no 'photo' or 'hash' param in response {} %s", coverUploadedResponse.toString());
                    if (callback.length > 0)
     ...
     */
    public Group callbackApi(CallbackApiSettings settings) throws JSONException {
        if (callbackApiHandler == null) {
     ...
     */
    public void setCallbackApiSettings(CallbackApiSettings settings) throws JSONException {
        callbackApiHandler = new CallbackApiHandler(settings);
        callbackApiHandler.setGroup(this);
    }
