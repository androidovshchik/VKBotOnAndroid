package rf.androidovshchik.vk_bot_android_sdk.utils;
...
public class Utils {
     ...
     * @return Query
     */
    public static String MapToURLParamsQuery(Object arr) throws JSONException {
        ...
        if (arr instanceof JSONObject) {
            ...
            Gson gson = new Gson();
            Map<?, ?> map = gson.fromJson(j.toString(), Map.class);
            boolean seen = false;
            String acc = null;
            for (Map.Entry<?, ?> p : map.entrySet()) {
                String s = p.getKey() + "=" + toQueryString(p.getValue());
                if (!seen) {
                    seen = true;
                    acc = s;
                } else {
     ...
     */
    public static String toQueryString(Object o) throws JSONException {
        ...
        if (o instanceof String[]) {
            return TextUtils.join(",", (String[]) o);
        }
    ...
    public static int sizeOfFile(String url, String dim) {
        ...
        try {
            ...
            Timber.e("IOException when calculating size of file {} , error: {} %s %s", url, ignored.toString());
        }
