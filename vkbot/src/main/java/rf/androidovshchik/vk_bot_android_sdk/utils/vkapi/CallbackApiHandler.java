...
import rf.androidovshchik.vk_bot_android_sdk.clients.Group;
import spark.Spark;
...
public class CallbackApiHandler {
    ...
    public CallbackApiHandler(String path) {
        ...
        int port = 80;
        Spark.port(port);
        Timber.i("Started listening to VK Callback API on port {}. %s", port + "");
        ...
        Spark.post(path, (request, response) -> {
            ...
            if (type.equals("confirmation")) {
                Timber.i("New confirmation request: {} %s", req.toString());
     ...
     * Non-default handler
     */
    public CallbackApiHandler(CallbackApiSettings settings) throws JSONException {
        ...
        Timber.i("Started listening to VK Callback API on port 80.");

        // Set server if it's not setted
        if (settings.getHost() == null || settings.getHost().length() < 2) {
            Timber.e("Server is not set: trying to set...");
            ...
            while (!server_ok) {
                ...
                Timber.e("New attempt to set server. Response: {} %s", response.toString());
                if (response.getString("state").equals("ok")) {
                    server_ok = true;
                    Timber.i("Server is installed.");
                }
            }
        }
     ...
     */
    private void handle(JSONObject request) throws JSONException {
