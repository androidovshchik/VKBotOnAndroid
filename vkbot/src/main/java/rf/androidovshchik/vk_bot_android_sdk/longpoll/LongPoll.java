...
import rf.androidovshchik.vk_bot_android_sdk.callbacks.AbstractCallback;
...
import rf.androidovshchik.vk_bot_android_sdk.longpoll.responses.GetLongPollServerResponse;
...
public class LongPoll {
    ...
    public LongPoll(Client client) {
        ...
        while (!dataSetted) {
            Timber.e("Some error occured when trying to get longpoll settings, aborting. Trying again in 1 sec.");
            try {
                Thread.sleep(1000);
        ...
        if (!longpollIsOn) {
            longpollIsOn = true;
            Thread threadLongpollListener = new Thread(() -> {
                try {
                    startListening();
    ...
    private boolean setData(Integer need_pts, Integer version, Double API, Integer wait, Integer mode) {
        ...
        try {
            serverResponse = getLongPollServer(client.getAccessToken());
        } catch (JSONException e) {
            e.printStackTrace();
            return false;
        }
        ...
        if (serverResponse == null) {
            Timber.e("Some error occured, bad response returned from getting LongPoll server settings (server, key, ts, pts).");
            return false;
        }
     ...
     */
    private GetLongPollServerResponse getLongPollServer(String access_token) throws JSONException {
        ...
        try {
            ...
            Timber.e("Bad responce of getting longpoll server.");
            return null;
        }

        Timber.i("GetLongPollServerResponse: \n{}\n %s", response.toString());
        ...
        if (!response.has("response") || !response.getJSONObject("response").has("key") || !response.getJSONObject("response").has("server") || !response.getJSONObject("response").has("ts")) {
            Timber.e("Bad response of getting longpoll server!\nQuery: {}\n Response: {} %s %s", query.toString(), response.toString());
            return null;
        }
     ...
     */
    private void startListening() throws JSONException {

        Timber.i("Started listening to events from VK LongPoll server...");
        ...
        while (longpollIsOn) {
            ...
            try {
                ...
                Timber.e("Some error occured, no updates got from longpoll server: {} %s", responseString);
                try {
                    Thread.sleep(1000);
            ...
            if (logUpdates) {
                Timber.i("Response of getting updates: \n{}\n %s", response.toString());
            }
            ...
            if (response.has("failed")) {
                ...
                Timber.e("Response of VK LongPoll fallen with error code {} %s", code + "");
                ...
                if (this.updatesHandler.callbacksCount() > 0 || this.updatesHandler.commandsCount() > 0 || this.updatesHandler.chatCallbacksCount() > 0) {
                    ...
                    if (response.has("ts") && response.has("updates")) {
                        ...
                        Timber.e("Bad response from VK LongPoll server: no `ts` or `updates` array: {} %s", response.toString());
                        try {
                            Thread.sleep(1000);
                        } catch (InterruptedException ignored) {
                        }
